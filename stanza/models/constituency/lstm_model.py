...
class LSTMModel(BaseModel, nn.Module):
    def __init__(self, pretrain, forward_charlm, backward_charlm, bert_model, bert_tokenizer, transitions, constituents, tags, words, rare_words, root_labels, constituent_opens, unary_limit, args):
        ...
        #
        # note that by transforming it to num_tree_lstm_layers * hidden_size,
        # the TREE_LSTM method has enough dimensions to fill out an initial hx
        # for multiple layers.  the non-TREE_LSTM methods wind up having more
        # information available at the final output
    ...
    def is_zero_decay_parameter(self, name):
    ...
    def is_low_decay_parameter(self, name):
        if self.is_zero_decay_parameter(name):
            return False
        if name.startswith("partitioned_transformer_module"):
            return True
        return False

    def is_base_decay_parameter(self, name):
        if self.is_zero_decay_parameter(name):
            return False
        if self.is_low_decay_parameter(name):
            return False
        return True

    def base_parameters(self):
        params = [param for name, param in self.named_parameters() if self.is_base_decay_parameter(name)]
        return params
    ...
    def zero_decay_parameters(self):
        params = [param for name, param in self.named_parameters() if self.is_zero_decay_parameter(name)]
        return params
